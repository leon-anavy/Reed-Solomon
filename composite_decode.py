import sys
from composite_RS import rs125_decoder,rs343_decoder,rs64_decoder,decode_oligo, rs256_decoder, rs3481_decoder
...
def main():
    ...
    try:
        ...
        verify_only = True
    file_out = file_in.replace('.dna','_decoded_veirfy_{}.dna'.format(verify_only))
    decode(file_in,file_out,alphabet,verify_only)
...
def decode(file_in,file_out,alphabet,verify_only=True):
    ...
    if alphabet == '4':
        ...
        partial_decode = partial(decode_oligo,rs_decoder=rs3481_decoder,verify_only=verify_only)
    elif alphabet == 'phi10':
        partial_decode = partial(decode_oligo,rs_decoder=rs256_decoder,verify_only=verify_only)    
    else:
